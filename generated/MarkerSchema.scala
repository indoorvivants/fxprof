...
class MarkerSchema private (private[fxprof] val args: MarkerSchemaArgs) {
  ...
  /** The label of how this marker should be displayed in the UI. If none is
    * provided, then the name is used.
    */
  def tooltipLabel: Option[String] = args.tooltipLabel

  /** This is how the marker shows up in the Marker Table description. If none
    * is provided, then the name is used.
    */
  def tableLabel: Option[String] = args.tableLabel

  /** This is how the marker shows up in the Marker Chart, where it is drawn on
    * the screen as a bar. If none is provided, then the name is used.
    */
  ...
  /** The fields that can be present on markers of this type. Not all listed
    * fields have to be present on every marker (they're all optional).
    */
  def fields: Vector[MarkerSchemaField] = args.fields

  /** An optional description for markers of this type. Will be displayed to the
    * user.
    */
  ...
  /** If present, specifies the key of a marker field that contains the marker's
    * color. The field should contain one of the GraphColor values. This allows
    * individual markers to have different colors based on their data.
    */
  def colorField: Option[String] = args.colorField

  /** If set to true, markers of this type are assumed to be well-nested with
    * all other stack-based markers on the same thread. Stack-based markers may
    * be displayed in a different part of the marker chart than non-stack-based
    * markers. Instant markers are always well-nested. For interval markers, or
    * for intervals defined by a start and an end marker, well-nested means
    * that, for all marker-defined timestamp intervals A and B, A either fully
    * encompasses B or is fully encompassed by B - there is no partial overlap.
    */
  ...
  given JsonValueCodec[MarkerSchema] =
    new JsonValueCodec {
      def decodeValue(in: JsonReader, default: MarkerSchema) =
        new MarkerSchema(
          summon[JsonValueCodec[MarkerSchemaArgs]].decodeValue(in, default.args)
        )
...
private[fxprof] object MarkerSchemaArgs {
  given JsonValueCodec[MarkerSchemaArgs] = JsonCodecMaker.make
}
