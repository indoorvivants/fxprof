...
class FileIoPayload private (private[fxprof] val args: FileIoPayloadArgs) {
  ...
  /** FileIO markers that are happening on the current thread don't have a
    * threadId, but they have threadId field if the markers belong to a
    * different (potentially non-profiled) thread. This field is added on
    * Firefox 78, but this is backwards compatible because previous FileIO
    * markers were also belonging to the threads they are in only. We still
    * don't serialize this field if the marker belongs to the thread they are
    * being captured.
    */
  ...
  override def equals(o: Any) = o.isInstanceOf[FileIoPayload] && o
    .asInstanceOf[FileIoPayload]
    .args
    .equals(this.args)
  ...
  implicit val codec: JsonValueCodec[FileIoPayload] =
    new JsonValueCodec[FileIoPayload] {
      def decodeValue(in: JsonReader, default: FileIoPayload) =
        new FileIoPayload(
          implicitly[JsonValueCodec[FileIoPayloadArgs]]
