...
class LongTaskMarkerPayload private (
    private[fxprof] val args: LongTaskMarkerPayloadArgs
) {
  ...
  def `withtype`(
      value: LongTaskMarkerPayload_Type.type
  ): LongTaskMarkerPayload =
    copy(_.copy(`type` = value))

  def withCategory(
      value: LongTaskMarkerPayload_Category.type
  ): LongTaskMarkerPayload =
  ...
  override def equals(o: Any) = o.isInstanceOf[LongTaskMarkerPayload] && o
    .asInstanceOf[LongTaskMarkerPayload]
    .args
    .equals(this.args)
...
object LongTaskMarkerPayload {
  ...
  def apply(
    ...
    new LongTaskMarkerPayload(
      LongTaskMarkerPayloadArgs(
        `type` = `type`,
        category = category
      )
    )
  implicit val codec: JsonValueCodec[LongTaskMarkerPayload] =
    new JsonValueCodec[LongTaskMarkerPayload] {
      def decodeValue(in: JsonReader, default: LongTaskMarkerPayload) =
        new LongTaskMarkerPayload(
          implicitly[JsonValueCodec[LongTaskMarkerPayloadArgs]]
