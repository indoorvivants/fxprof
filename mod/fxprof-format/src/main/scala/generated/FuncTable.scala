...
/** The funcTable stores the functions that were called in the profile. These
  * can be native functions (e.g. C / C++ / rust), JavaScript functions, or
  * "label" functions. Multiple frames can have the same function: The frame
  * represents which part of a function was being executed at a given moment,
  * and the function groups all frames that occurred inside that function.
  * Concretely, for native code, each encountered instruction address is a
  * separate frame, and the function groups all instruction addresses which were
  * symbolicated with the same function name. For JS code, each encountered
  * line/column in a JS file is a separate frame, and the function represents an
  * entire JS function which can span multiple lines. * Funcs that are orphaned,
  * i.e. funcs that no frame refers to, do not have meaningful values in their
  * fields. Symbolication will cause many funcs that were created upfront to
  * become orphaned, as the frames that originally referred to them get
  * reassigned to the canonical func for their actual function.
  */
class FuncTable private (private[fxprof] val args: FuncTableArgs) {
  ...
  /** isJS and relevantForJS describe the function type. Non-JavaScript
    * functions can be marked as "relevant for JS" so that for example DOM API
    * label functions will show up in any JavaScript stack views. It may be
    * worth combining these two fields into one:
  ...
  /** The resource describes "Which bag of code did this function come from?".
    ...
    * url. For native functions, the resource is of type library. For labels and
    * for other unidentified functions, we set the resource to -1.
    */
  ...
  /** These are non-null for JS functions only. The line and column describe the
    * location of the *start* of the JS function. As for the information about
    * which which lines / columns inside the function were actually hit during
    * execution, that information is stored in the frameTable, not in the
    * funcTable.
    */
  ...
  override def equals(o: Any) = o
    .isInstanceOf[FuncTable] && o.asInstanceOf[FuncTable].args.equals(this.args)
  ...
  implicit val codec: JsonValueCodec[FuncTable] =
    new JsonValueCodec[FuncTable] {
      def decodeValue(in: JsonReader, default: FuncTable) =
        new FuncTable(
          implicitly[JsonValueCodec[FuncTableArgs]]
