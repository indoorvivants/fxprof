...
class UrlMarkerPayload private (
    private[fxprof] val args: UrlMarkerPayloadArgs
) {
  ...
  override def equals(o: Any) = o.isInstanceOf[UrlMarkerPayload] && o
    .asInstanceOf[UrlMarkerPayload]
    .args
    .equals(this.args)
...
object UrlMarkerPayload {
  ...
  def apply(
    ...
    new UrlMarkerPayload(
      UrlMarkerPayloadArgs(
        `type` = `type`,
        url = url
      )
    )
  implicit val codec: JsonValueCodec[UrlMarkerPayload] =
    new JsonValueCodec[UrlMarkerPayload] {
      def decodeValue(in: JsonReader, default: UrlMarkerPayload) =
        new UrlMarkerPayload(
          implicitly[JsonValueCodec[UrlMarkerPayloadArgs]]
