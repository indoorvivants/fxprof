...
class RawSamplesTable private (private[fxprof] val args: RawSamplesTableArgs) {
  ...
  /** Responsiveness is the older version of eventDelay. It injects events every
    * 16ms. This is optional because newer profiles don't have that field
    * anymore.
    */
  def responsiveness: Option[Vector[Option[Milliseconds]]] = args.responsiveness

  /** Event delay is the newer version of responsiveness. It allow us to get a
    * finer-grained view of jank by inferring what would be the delay of a
    * hypothetical input event at any point in time. It requires a
    * pre-processing to be able to visualize properly. This is optional because
    * older profiles didn't have that field.
    */
  ...
  /** If the `time` column is not present, then the `timeDeltas` column must be
    * present.
    */
  ...
  /** CPU usage value of the current thread. Its values are null only if the
    * back-end fails to get the CPU usage from operating system. It's landed in
    * Firefox 86, and it is optional because older profile versions may not have
    * it or that feature could be disabled. No upgrader was written for this
    * change because it's a completely new data source. The first value is
    * ignored - it's not meaningful because there is no previous sample.
    */
  ...
  /** Setter for [[responsiveness]] field
    ...
    * anymore.
    */
  def withResponsiveness(
      value: Option[Vector[Option[Milliseconds]]]
  ): RawSamplesTable =
  ...
  /** Setter for [[threadCPUDelta]] field
    ...
    * ignored - it's not meaningful because there is no previous sample.
    */
  def withThreadCPUDelta(
      value: Option[Vector[Option[Double]]]
  ): RawSamplesTable =
  ...
  override def equals(o: Any) = o.isInstanceOf[RawSamplesTable] && o
    .asInstanceOf[RawSamplesTable]
    .args
    .equals(this.args)
  ...
  implicit val codec: JsonValueCodec[RawSamplesTable] =
    new JsonValueCodec[RawSamplesTable] {
      def decodeValue(in: JsonReader, default: RawSamplesTable) =
        new RawSamplesTable(
          implicitly[JsonValueCodec[RawSamplesTableArgs]]
