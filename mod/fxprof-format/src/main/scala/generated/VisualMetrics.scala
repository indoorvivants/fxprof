...
/** Visual metrics are performance metrics that measure above-the-fold webpage
  * visual performance, and more accurately describe how human end-users
  * perceive the speed of webpage loading. These metrics serves as additional
  * metrics to the typical page-level metrics such as onLoad. Visual metrics
  * contains key metrics such as Speed Index, Perceptual Speed Index, and
  * ContentfulSpeedIndex, along with their visual progression. These properties
  * find their way into the gecko profile through running browsertime, which is
  * a tool that lets you collect performance metrics of your website.
  * Browsertime provides the option of generating a gecko profile, which then
  * embeds these visual metrics into the geckoprofile. More information about
  * browsertime can be found through https://github.com/sitespeedio/browsertime.
  * These values are generated only when browsertime is run with
  * --visualMetrics. * Most values from this object are generated by the python
  * script
  * https://github.com/sitespeedio/browsertime/blob/6e88284930c1d3ded8d9d95252d2e13c252d361c/browsertime/visualmetrics.py
  * Especially look at the function calculate_visual_metrics. Then the script is
  * called and the result processed from the JS files in
  * https://github.com/sitespeedio/browsertime/tree/6e88284930c1d3ded8d9d95252d2e13c252d361c/lib/video/postprocessing/visualmetrics
  * and
  * https://github.com/sitespeedio/browsertime/blob/6e88284930c1d3ded8d9d95252d2e13c252d361c/lib/core/engine/iteration.js#L261-L264.
  * Finally they're inserted into the JSON profile in
  * https://github.com/sitespeedio/browsertime/blob/6e88284930c1d3ded8d9d95252d2e13c252d361c/lib/firefox/webdriver/firefox.js#L215-L230
  */
class VisualMetrics private (private[fxprof] val args: VisualMetricsArgs) {
  ...
  def ContentfulSpeedIndexProgress: Option[Vector[ProgressGraphData]] =
    args.ContentfulSpeedIndexProgress

  def PerceptualSpeedIndex: Option[Double] = args.PerceptualSpeedIndex

  def PerceptualSpeedIndexProgress: Option[Vector[ProgressGraphData]] =
    args.PerceptualSpeedIndexProgress
  ...
  /** Setter for [[ContentfulSpeedIndex]] field
    *
    * Contentful and Perceptual values may be missing. They're generated only if
    * the user specifies the options --visualMetricsContentful and
    * --visualMetricsPerceptual in addition to --visualMetrics.
  ...
  def withContentfulSpeedIndexProgress(
      value: Option[Vector[ProgressGraphData]]
  ): VisualMetrics =
  ...
  def withPerceptualSpeedIndexProgress(
      value: Option[Vector[ProgressGraphData]]
  ): VisualMetrics =
    copy(_.copy(PerceptualSpeedIndexProgress = value))

  /** Setter for [[VisualReadiness]] field
    *
    * VisualReadiness and VisualCompleteXX values are generated in
    * https://github.com/sitespeedio/browsertime/blob/main/lib/video/postprocessing/visualmetrics/extraMetrics.js
    */
  ...
  override def equals(o: Any) = o.isInstanceOf[VisualMetrics] && o
    .asInstanceOf[VisualMetrics]
    .args
    .equals(this.args)
...
object VisualMetrics {
  ...
  /** Construct a [[VisualMetrics]]
    ...
    * @param SpeedIndex
    * @param VisualReadinessVisualReadiness
    *   and VisualCompleteXX values are generated in
    *   https://github.com/sitespeedio/browsertime/blob/main/lib/video/postprocessing/visualmetrics/extraMetrics.js
  ...
  implicit val codec: JsonValueCodec[VisualMetrics] =
    new JsonValueCodec[VisualMetrics] {
      def decodeValue(in: JsonReader, default: VisualMetrics) =
        new VisualMetrics(
          implicitly[JsonValueCodec[VisualMetricsArgs]]
