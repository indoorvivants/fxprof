...
class IPCMarkerPayload private (
    private[fxprof] val args: IPCMarkerPayloadArgs
) {
  ...
  /** Phase is not present in older profiles (in this case the phase is
    * "endpoint").
    */
  ...
  /** `tid` of the thread that this marker is originated from. It is undefined
    * when the IPC marker is originated from the same thread. Also, this field
    * is added in Firefox 100. It will always be undefined for the older
    * profiles.
    */
  ...
  /** These fields are added in the deriving process from `IPCSharedData`, and
    ...
    * message. We mark these fields as optional because we represent non-derived
    * markers and derived markers with the same type. These fields are always
    * present on derived markers and never present on non-derived markers.
    */
  ...
  override def equals(o: Any) = o.isInstanceOf[IPCMarkerPayload] && o
    .asInstanceOf[IPCMarkerPayload]
    .args
    .equals(this.args)
...
object IPCMarkerPayload {
  ...
  /** Construct a [[IPCMarkerPayload]]
    ...
    * @param startTime
    * @param endTime
    * @param otherPidotherPid
    *   is a string in the processed format.
    * @param messageType
    * @param messageSeqno
    * @param side
    * @param direction
  ...
  def apply(
    ...
    new IPCMarkerPayload(
      IPCMarkerPayloadArgs(
        `type` = `type`,
        startTime = startTime,
        endTime = endTime,
        otherPid = otherPid,
  ...
  implicit val codec: JsonValueCodec[IPCMarkerPayload] =
    new JsonValueCodec[IPCMarkerPayload] {
      def decodeValue(in: JsonReader, default: IPCMarkerPayload) =
        new IPCMarkerPayload(
          implicitly[JsonValueCodec[IPCMarkerPayloadArgs]]
