...
/** This object represents the configuration of the profiler when the profile
  * was recorded.
  */
class ProfilerConfiguration private (
    private[fxprof] val args: ProfilerConfigurationArgs
) {
  ...
  /** Optional because that field is introduced in Firefox 72. Active Tab ID
    * indicates a Firefox tab. `0` means null value. Firefox only outputs `0`
    * and not null, that's why we should take care of this case while we are
    * consuming it. If it's `0`, we should revert back to the full view since
    * there isn't enough data to show the active tab view.
    */
  ...
  override def equals(o: Any) = o.isInstanceOf[ProfilerConfiguration] && o
    .asInstanceOf[ProfilerConfiguration]
    .args
    .equals(this.args)
  ...
  implicit val codec: JsonValueCodec[ProfilerConfiguration] =
    new JsonValueCodec[ProfilerConfiguration] {
      def decodeValue(in: JsonReader, default: ProfilerConfiguration) =
        new ProfilerConfiguration(
          implicitly[JsonValueCodec[ProfilerConfigurationArgs]]
