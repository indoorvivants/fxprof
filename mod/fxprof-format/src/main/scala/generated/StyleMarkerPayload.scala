...
class StyleMarkerPayload private (
    private[fxprof] val args: StyleMarkerPayloadArgs
) {
  ...
  def withCategory(
      value: StyleMarkerPayload_Category.type
  ): StyleMarkerPayload =
  ...
  override def equals(o: Any) = o.isInstanceOf[StyleMarkerPayload] && o
    .asInstanceOf[StyleMarkerPayload]
    .args
    .equals(this.args)
...
object StyleMarkerPayload {
  ...
  def apply(
    ...
    new StyleMarkerPayload(
      StyleMarkerPayloadArgs(
        `type` = `type`,
        category = category,
        cause = None,
  ...
  implicit val codec: JsonValueCodec[StyleMarkerPayload] =
    new JsonValueCodec[StyleMarkerPayload] {
      def decodeValue(in: JsonReader, default: StyleMarkerPayload) =
        new StyleMarkerPayload(
          implicitly[JsonValueCodec[StyleMarkerPayloadArgs]]
