...
/** This variant is the original version of the table, before the memory address
  ...
  */
class UnbalancedNativeAllocationsTable private (
    private[fxprof] val args: UnbalancedNativeAllocationsTableArgs
) {
  def time: Vector[Milliseconds] = args.time

  /** "weight" is used here rather than "bytes", so that this type will match
    * the SamplesLikeTableShape.
    */
  def weight: Vector[Bytes] = args.weight

  def weightType: UnbalancedNativeAllocationsTable_WeightType.type =
    args.weightType
  ...
  def withWeightType(
      value: UnbalancedNativeAllocationsTable_WeightType.type
  ): UnbalancedNativeAllocationsTable =
    copy(_.copy(weightType = value))

  def withStack(
      value: Vector[Option[IndexIntoStackTable]]
  ): UnbalancedNativeAllocationsTable =
  ...
  private def copy(
      f: UnbalancedNativeAllocationsTableArgs => UnbalancedNativeAllocationsTableArgs
  ) =
    new UnbalancedNativeAllocationsTable(f(args))

  override def equals(o: Any) =
    o.isInstanceOf[UnbalancedNativeAllocationsTable] && o
      .asInstanceOf[UnbalancedNativeAllocationsTable]
      .args
      .equals(this.args)
...
object UnbalancedNativeAllocationsTable {
  ...
  def apply(
    ...
    new UnbalancedNativeAllocationsTable(
      UnbalancedNativeAllocationsTableArgs(
        time = Vector.empty,
        weight = Vector.empty,
  ...
  implicit val codec: JsonValueCodec[UnbalancedNativeAllocationsTable] =
    new JsonValueCodec[UnbalancedNativeAllocationsTable] {
      def decodeValue(
          in: JsonReader,
          default: UnbalancedNativeAllocationsTable
      ) =
        new UnbalancedNativeAllocationsTable(
          implicitly[JsonValueCodec[UnbalancedNativeAllocationsTableArgs]]
