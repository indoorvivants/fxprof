...
/** Markers represent arbitrary events that happen within the browser. They have
  * a name, time, and potentially a JSON data payload. These can come from all
  * over the system. For instance Paint markers instrument the rendering and
  * layout process. Engineers can easily add arbitrary markers to their code
  * without coordinating with profiler.firefox.com to instrument their code. *
  * In the profile, these markers are raw and unprocessed. In the marker
  * selectors, we can run them through a processing pipeline to match up start
  * and end markers to create markers with durations, or even take a string-only
  * marker and parse it into a structured marker.
  */
class RawMarkerTable private (private[fxprof] val args: RawMarkerTableArgs) {
  ...
  override def equals(o: Any) = o.isInstanceOf[RawMarkerTable] && o
    .asInstanceOf[RawMarkerTable]
    .args
    .equals(this.args)
  ...
  implicit val codec: JsonValueCodec[RawMarkerTable] =
    new JsonValueCodec[RawMarkerTable] {
      def decodeValue(in: JsonReader, default: RawMarkerTable) =
        new RawMarkerTable(
          implicitly[JsonValueCodec[RawMarkerTableArgs]]
