...
class PrefMarkerPayload private (
    private[fxprof] val args: PrefMarkerPayloadArgs
) {
  ...
  override def equals(o: Any) = o.isInstanceOf[PrefMarkerPayload] && o
    .asInstanceOf[PrefMarkerPayload]
    .args
    .equals(this.args)
...
object PrefMarkerPayload {
  ...
  def apply(
    ...
    new PrefMarkerPayload(
      PrefMarkerPayloadArgs(
        `type` = `type`,
  ...
  implicit val codec: JsonValueCodec[PrefMarkerPayload] =
    new JsonValueCodec[PrefMarkerPayload] {
      def decodeValue(in: JsonReader, default: PrefMarkerPayload) =
        new PrefMarkerPayload(
          implicitly[JsonValueCodec[PrefMarkerPayloadArgs]]
...
private[fxprof] object PrefMarkerPayloadArgs {
  implicit val codec: JsonValueCodec[PrefMarkerPayloadArgs] =
