...
class ProfileMeta private (private[fxprof] val args: ProfileMetaArgs) {
  ...
  def startTimeAsClockMonotonicNanosecondsSinceBoot: Option[Double] =
    args.startTimeAsClockMonotonicNanosecondsSinceBoot

  def startTimeAsMachAbsoluteTimeNanoseconds: Option[Double] =
    args.startTimeAsMachAbsoluteTimeNanoseconds

  def startTimeAsQueryPerformanceCounterValue: Option[Double] =
    args.startTimeAsQueryPerformanceCounterValue
  ...
  /** The process type where the Gecko profiler was started. This is the raw
    * enum numeric value as defined here:
  ...
  /** The extensions property landed in Firefox 60, and is only optional because
    * older processed profile versions may not have it. No upgrader was written
    * for this change.
    */
  def extensions: Option[ExtensionTable] = args.extensions

  /** The list of categories used in this profile. If present, it must contain
    * at least the "default category" which is defined as the first category
    * whose color is "grey" - this category usually has the name "Other". If
    * meta.categories is not present, a default list is substituted.
    */
  ...
  /** This value represents a boolean, but for some reason is written out as an
    * int value. It's 0 for the stack walking feature being turned off, and 1
    * for stackwalking being turned on.
    */
  def stackwalk: ProfileMeta_Stackwalk = args.stackwalk

  /** A boolean flag indicating whether the profiled application is using a
    * debug build. It's false for opt builds, and true for debug builds. This
    * property is optional because older processed profiles don't have this but
    * this property was added to Firefox a long time ago. It should work on
    * older Firefox versions without any problem.
    */
  def debug: Option[Boolean] = args.debug

  /** This is the Gecko profile format version (the unprocessed version received
  ...
  /** The XPCOM ABI (Application Binary Interface) name, taking the form:
    * {CPU_ARCH}-{TARGET_COMPILER_ABI} e.g. "x86_64-gcc3" See
    * https://developer.mozilla.org/en-US/docs/Mozilla/Tech/XPCOM/XPCOM_ABI
    */
  def abi: Option[String] = args.abi

  /** The "misc" value of the browser's user agent, typically the revision of
    * the browser. e.g. "rv:63.0", which would be Firefox 63.0 See
    * https://searchfox.org/mozilla-central/rev/819cd31a93fd50b7167979607371878c4d6f18e8/netwerk/protocol/http/nsHttpHandler.h#543
    */
  ...
  /** The widget toolkit used for GUI rendering. Older versions of Firefox for
    * Linux had the 2 flavors gtk2/gtk3, and so we could find the value "gtk3".
    */
  ...
  /** A boolean flag indicating whether we symbolicated this profile. If this is
    * false we'll start a symbolication process when the profile is loaded. A
    * missing property means that it's an older profile, it stands for an
  ...
  /** A boolean flag indicating that symbolication is not supported Used for
    * imported profiles that cannot be symbolicated
    */
  def symbolicationNotSupported: Option[Boolean] =
    args.symbolicationNotSupported

  /** Visual metrics contains additional performance metrics such as Speed
    * Index, Perceptual Speed Index, and ContentfulSpeedIndex. This is optional
    * because only profiles generated by browsertime will have this property.
    * Source code for browsertime can be found at
    * https://github.com/sitespeedio/browsertime.
    */
  def visualMetrics: Option[VisualMetrics] = args.visualMetrics

  /** The configuration of the profiler at the time of recording. Optional since
    * older versions of Firefox did not include it.
    */
  ...
  /** Information of the device that profile is captured from. Currently it's
    * only present for Android devices and it includes brand and model names of
    * that device. It's optional because profiles from non-Android devices and
    * from older Firefox versions may not have it. This property landed in
    * Firefox 88.
    */
  def device: Option[String] = args.device

  /** Profile importers can optionally add information about where they are
    * imported from. They also use the "product" field in the meta information,
    * but this is somewhat ambiguous. This field, if present, is unambiguous
  ...
  /** Hide the "Look up the function name on Searchfox" menu entry?
    */
  def sourceCodeIsNotOnSearchfox: Option[Boolean] =
    args.sourceCodeIsNotOnSearchfox

  /** Indexes of the threads that are initially visible in the UI. This is
    * useful for imported profiles for which the internal visibility score
    * ranking does not make sense.
    */
  def initialVisibleThreads: Option[Vector[ThreadIndex]] =
    args.initialVisibleThreads

  /** Indexes of the threads that are initially selected in the UI. This is also
    * most useful for imported profiles where just using the first thread of
    * each process might not make sense.
    */
  def initialSelectedThreads: Option[Vector[ThreadIndex]] =
    args.initialSelectedThreads
  ...
  def withStartTimeAsClockMonotonicNanosecondsSinceBoot(
      value: Option[Double]
  ): ProfileMeta =
    copy(_.copy(startTimeAsClockMonotonicNanosecondsSinceBoot = value))

  def withStartTimeAsMachAbsoluteTimeNanoseconds(
      value: Option[Double]
  ): ProfileMeta =
    copy(_.copy(startTimeAsMachAbsoluteTimeNanoseconds = value))

  def withStartTimeAsQueryPerformanceCounterValue(
      value: Option[Double]
  ): ProfileMeta =
  ...
  /** Setter for [[initialVisibleThreads]] field
    ...
    * ranking does not make sense.
    */
  def withInitialVisibleThreads(
      value: Option[Vector[ThreadIndex]]
  ): ProfileMeta =
  ...
  /** Setter for [[initialSelectedThreads]] field
    ...
    * each process might not make sense.
    */
  def withInitialSelectedThreads(
      value: Option[Vector[ThreadIndex]]
  ): ProfileMeta =
  ...
  override def equals(o: Any) = o.isInstanceOf[ProfileMeta] && o
    .asInstanceOf[ProfileMeta]
    .args
    .equals(this.args)
...
object ProfileMeta {
  ...
  /** Construct a [[ProfileMeta]]
    * @param intervalThe
    *   interval at which the threads are sampled.
    * @param startTimeWhen
    *   the main process started. Timestamp expressed in milliseconds since
    *   midnight January 1, 1970 GMT.
    * @param processTypeThe
    *   process type where the Gecko profiler was started. This is the raw enum
    *   numeric value as defined here:
    *   https://searchfox.org/mozilla-central/rev/819cd31a93fd50b7167979607371878c4d6f18e8/xpcom/build/nsXULAppAPI.h#365
    * @param productThe
    *   name of the product, most likely "Firefox".
    * @param stackwalkThis
    *   value represents a boolean, but for some reason is written out as an int
    *   value. It's 0 for the stack walking feature being turned off, and 1 for
    *   stackwalking being turned on.
    * @param versionThis
    *   is the Gecko profile format version (the unprocessed version received
    *   directly from the browser.)
    * @param preprocessedProfileVersionThis
    *   is the processed profile format version.
    */
  def apply(
  ...
  implicit val codec: JsonValueCodec[ProfileMeta] =
    new JsonValueCodec[ProfileMeta] {
      def decodeValue(in: JsonReader, default: ProfileMeta) =
        new ProfileMeta(
          implicitly[JsonValueCodec[ProfileMetaArgs]]
