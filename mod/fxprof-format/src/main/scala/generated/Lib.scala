...
class Lib private (private[fxprof] val args: LibArgs) {
  ...
  /** The codeId is currently always null. In the future, it will have the
    * following values:
    *   - On macOS, it will still be null.
    *   - On Linux / Android, it will have the full GNU build id. (The
    *     breakpadId is also based on the build id, but truncates some
    *     information.) This lets us obtain unstripped system libraries on Linux
    *     distributions which have a "debuginfod" server, and we can use those
    *     unstripped binaries for symbolication.
    *   - On Windows, it will be the codeId for the binary (.exe / .dll), as
    *     used by Windows symbol servers. This will allow us to get assembly
    *     code for Windows system libraries for profiles which were captured on
    *     another machine.
    */
  ...
  override def equals(o: Any) =
    o.isInstanceOf[Lib] && o.asInstanceOf[Lib].args.equals(this.args)
  ...
  implicit val codec: JsonValueCodec[Lib] =
    new JsonValueCodec[Lib] {
      def decodeValue(in: JsonReader, default: Lib) =
        new Lib(
          implicitly[JsonValueCodec[LibArgs]].decodeValue(in, default.args)
        )
