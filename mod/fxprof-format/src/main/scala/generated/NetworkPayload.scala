...
/** Network http/https loads - one marker for each load that reaches the STOP
  * state that occurs, plus one for the initial START of the load, with the URI
  * and the status. A unique ID is included to allow these to be linked. Note
  * that the 'name' field currently also has the id ("Load N") so that marker.js
  * will not merge separate loads of the same URI. Note also that URI is not
  * necessarily included in later network markers for a specific load to avoid
  * having to use cycles during collection to access, allocate and copy the URI.
  * Markers using the same ID are all for the same load. * Most of the fields
  * only are included on STOP, and not all of them may be included depending on
  * what states happen during the load. Also note that redirects are logged as
  * well.
  */
class NetworkPayload private (private[fxprof] val args: NetworkPayloadArgs) {
  ...
  /** contentType is the value of the Content-Type header from the HTTP
    * response. An empty string means the response had no content type, while a
    * value of null means no HTTP response was received. If this property is
    * absent then it means this profiler came from an older version of the Gecko
    * profiler without content type support.
    */
  ...
  /** If present and true, this network marker originated from a request in a
    * private browsing session. Most markers tied to a window also have a
    * innerWindowID property, but that's not always the case, especially for the
    * requests for the top level navigation. That's why this property is needed
    * in addition to the innerWindowID property that we also have. It's always
    * absent in Firefox < 98 because we couldn't capture private browsing data
    * back then.
    */
  ...
  /** Used to express class dependencies and characteristics. Possible flags:
    * Leader, Follower, Speculative, Background, Unblocked, Throttleable,
    * UrgentStart, DontThrottle, Tail, TailAllowed, and TailForbidden. Multiple
    * flags can be set, separated by '|', or we use 'Unset' if no flag is set.
    */
  ...
  /** fetchStart doesn't exist directly in raw markers. This is added in the
    * deriving process and represents the junction between START and END
    * markers. This is the same value as the start marker's endTime and the end
    * marker's startTime (which are the same values). We don't expose it
    * directly but this is useful for debugging.
    */
  ...
  /** The following properties are present only in non-START markers.
    ...
    * `tcpConnectEnd`, `secureConnectionStart`, and `connectEnd`. NOTE: If you
    * add a new property, don't forget to adjust its timestamp in
  ...
  override def equals(o: Any) = o.isInstanceOf[NetworkPayload] && o
    .asInstanceOf[NetworkPayload]
    .args
    .equals(this.args)
...
object NetworkPayload {
  ...
  /** Construct a [[NetworkPayload]]
    ...
    * @param pri
    * @param statusSee
    *   all possible values in tools/profiler/core/platform.cpp
    * @param startTimestartTime
    *   is when the channel opens. This happens on the process' main thread.
    * @param endTimeendTime
    *   is the time when the response is sent back to the caller, this happens
  ...
  implicit val codec: JsonValueCodec[NetworkPayload] =
    new JsonValueCodec[NetworkPayload] {
      def decodeValue(in: JsonReader, default: NetworkPayload) =
        new NetworkPayload(
          implicitly[JsonValueCodec[NetworkPayloadArgs]]
