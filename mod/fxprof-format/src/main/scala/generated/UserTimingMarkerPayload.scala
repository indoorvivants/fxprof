...
/** The payload for the UserTimings API. These are added through
  * performance.measure() and performance.mark().
  * https://developer.mozilla.org/en-US/docs/Web/API/Performance
  */
class UserTimingMarkerPayload private (
    private[fxprof] val args: UserTimingMarkerPayloadArgs
) {
  ...
  def `withtype`(
      value: UserTimingMarkerPayload_Type.type
  ...
  def withEntryType(
      value: UserTimingMarkerPayload_EntryType
  ): UserTimingMarkerPayload =
    copy(_.copy(entryType = value))

  private def copy(
      f: UserTimingMarkerPayloadArgs => UserTimingMarkerPayloadArgs
  ) =
    new UserTimingMarkerPayload(f(args))

  override def equals(o: Any) = o.isInstanceOf[UserTimingMarkerPayload] && o
    .asInstanceOf[UserTimingMarkerPayload]
    .args
    .equals(this.args)
...
object UserTimingMarkerPayload {
  ...
  def apply(
    ...
    new UserTimingMarkerPayload(
      UserTimingMarkerPayloadArgs(
        `type` = `type`,
        name = name,
        entryType = entryType
      )
  ...
  implicit val codec: JsonValueCodec[UserTimingMarkerPayload] =
    new JsonValueCodec[UserTimingMarkerPayload] {
      def decodeValue(in: JsonReader, default: UserTimingMarkerPayload) =
        new UserTimingMarkerPayload(
          implicitly[JsonValueCodec[UserTimingMarkerPayloadArgs]]
