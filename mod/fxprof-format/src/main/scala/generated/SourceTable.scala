...
/** Table containing source code file references in the processed profile
  * format. Maps UUIDs to filenames for JavaScript sources, while native code
  * sources have null UUIDs. Native code sources are added during the
  * symbolication process. This allows the profiler to display source code
  * context when viewing frames. This is an expanded version of GeckoSourceTable
  * that includes native code sources in addition to JavaScript sources.
  */
class SourceTable private (private[fxprof] val args: SourceTableArgs) {
  ...
  override def equals(o: Any) = o.isInstanceOf[SourceTable] && o
    .asInstanceOf[SourceTable]
    .args
    .equals(this.args)
  ...
  implicit val codec: JsonValueCodec[SourceTable] =
    new JsonValueCodec[SourceTable] {
      def decodeValue(in: JsonReader, default: SourceTable) =
        new SourceTable(
          implicitly[JsonValueCodec[SourceTableArgs]]
