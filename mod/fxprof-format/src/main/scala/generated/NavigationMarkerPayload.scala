...
class NavigationMarkerPayload private (
    private[fxprof] val args: NavigationMarkerPayloadArgs
) {
  ...
  def `withtype`(
      value: NavigationMarkerPayload_Type.type
  ): NavigationMarkerPayload =
    copy(_.copy(`type` = value))

  def withCategory(
      value: NavigationMarkerPayload_Category.type
  ): NavigationMarkerPayload =
  ...
  private def copy(
      f: NavigationMarkerPayloadArgs => NavigationMarkerPayloadArgs
  ) =
    new NavigationMarkerPayload(f(args))

  override def equals(o: Any) = o.isInstanceOf[NavigationMarkerPayload] && o
    .asInstanceOf[NavigationMarkerPayload]
    .args
    .equals(this.args)
...
object NavigationMarkerPayload {
  ...
  def apply(
    ...
    new NavigationMarkerPayload(
      NavigationMarkerPayloadArgs(
        `type` = `type`,
        category = category,
        eventType = None,
        innerWindowID = None
  ...
  implicit val codec: JsonValueCodec[NavigationMarkerPayload] =
    new JsonValueCodec[NavigationMarkerPayload] {
      def decodeValue(in: JsonReader, default: NavigationMarkerPayload) =
        new NavigationMarkerPayload(
          implicitly[JsonValueCodec[NavigationMarkerPayloadArgs]]
