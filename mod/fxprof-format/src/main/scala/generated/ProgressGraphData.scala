...
/** Visual progress describes the visual progression during page load. A sample
  * is generated everytime the visual completeness of the webpage changes.
  */
class ProgressGraphData private (
    private[fxprof] val args: ProgressGraphDataArgs
) {

  /** A percentage that describes the visual completeness of the webpage,
    * ranging from 0% - 100%
    */
  def percent: Double = args.percent

  /** The time in milliseconds which the sample was taken. This can be null due
    * to https://github.com/sitespeedio/browsertime/issues/1746.
    */
  ...
  override def equals(o: Any) = o.isInstanceOf[ProgressGraphData] && o
    .asInstanceOf[ProgressGraphData]
    .args
    .equals(this.args)
...
object ProgressGraphData {
  ...
  /** Construct a [[ProgressGraphData]]
    * @param percentA
    *   percentage that describes the visual completeness of the webpage,
    *   ranging from 0% - 100%
    */
  def apply(
      percent: Double
  ): ProgressGraphData =
    new ProgressGraphData(
      ProgressGraphDataArgs(
        percent = percent,
        timestamp = None
      )
    )
  implicit val codec: JsonValueCodec[ProgressGraphData] =
    new JsonValueCodec[ProgressGraphData] {
      def decodeValue(in: JsonReader, default: ProgressGraphData) =
        new ProgressGraphData(
          implicitly[JsonValueCodec[ProgressGraphDataArgs]]
