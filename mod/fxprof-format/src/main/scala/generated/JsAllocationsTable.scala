...
/** JS allocations are recorded as a marker payload, but in profile processing
  * they are moved to the Thread. This allows them to be part of the stack
  * processing pipeline.
  */
class JsAllocationsTable private (
    private[fxprof] val args: JsAllocationsTableArgs
) {
  ...
  def withWeightType(
      value: JsAllocationsTable_WeightType.type
  ): JsAllocationsTable =
  ...
  override def equals(o: Any) = o.isInstanceOf[JsAllocationsTable] && o
    .asInstanceOf[JsAllocationsTable]
    .args
    .equals(this.args)
...
object JsAllocationsTable {
  ...
  def apply(
    ...
    new JsAllocationsTable(
      JsAllocationsTableArgs(
        time = Vector.empty,
  ...
  implicit val codec: JsonValueCodec[JsAllocationsTable] =
    new JsonValueCodec[JsAllocationsTable] {
      def decodeValue(in: JsonReader, default: JsAllocationsTable) =
        new JsAllocationsTable(
          implicitly[JsonValueCodec[JsAllocationsTableArgs]]
