...
/** The stack table stores the tree of stack nodes of a thread. The shape of the
  * tree is encoded in the prefix column: Root stack nodes have null as their
  * prefix, and every non-root stack has the stack index of its "caller" /
  * "parent" as its prefix. Every stack node also has a frame and a category. A
  * "call stack" is a list of frames. Every stack index in the stack table
  * represents such a call stack; the "list of frames" is obtained by walking
  * the path in the tree from the root to the given stack node. * Stacks are
  * used in the thread's samples; each sample refers to a stack index. Stacks
  * can be shared between samples. * With this representation, every sample only
  * needs to store a single integer to identify the sample's stack. We take
  * advantage of the fact that many call stacks in the profile have a shared
  * prefix; storing these stacks as a tree saves a lot of space compared to
  * storing them as actual lists of frames.
  */
class RawStackTable private (private[fxprof] val args: RawStackTableArgs) {
  ...
  override def equals(o: Any) = o.isInstanceOf[RawStackTable] && o
    .asInstanceOf[RawStackTable]
    .args
    .equals(this.args)
...
object RawStackTable {
  ...
  implicit val codec: JsonValueCodec[RawStackTable] =
    new JsonValueCodec[RawStackTable] {
      def decodeValue(in: JsonReader, default: RawStackTable) =
        new RawStackTable(
          implicitly[JsonValueCodec[RawStackTableArgs]]
