...
/** The nativeSymbols table stores the addresses and symbol names for all
  * symbols that were encountered by frame addresses in this thread. This table
  * can contain symbols from multiple libraries, and the symbols are in
  * arbitrary order. Note: Despite the similarity in name, this table is not
  ...
  * libraries.
  */
class NativeSymbolTable private (
    private[fxprof] val args: NativeSymbolTableArgs
) {
  ...
  override def equals(o: Any) = o.isInstanceOf[NativeSymbolTable] && o
    .asInstanceOf[NativeSymbolTable]
    .args
    .equals(this.args)
...
object NativeSymbolTable {
  ...
  def apply(
    ...
    new NativeSymbolTable(
      NativeSymbolTableArgs(
  ...
  implicit val codec: JsonValueCodec[NativeSymbolTable] =
    new JsonValueCodec[NativeSymbolTable] {
      def decodeValue(in: JsonReader, default: NativeSymbolTable) =
        new NativeSymbolTable(
          implicitly[JsonValueCodec[NativeSymbolTableArgs]]
