...
class Category private (private[fxprof] val args: CategoryArgs) {
  ...
  /** The category color. Must be picked from the CategoryColor list. At least
    * one category with color "grey" must be present in the category list.
    */
  def color: CategoryColor = args.color

  /** The list of subcategories. Must always have at least one element;
    * subcategory zero must be the "Other" subcategory and is used to refer to
    * the category itself.
    */
  ...
  override def equals(o: Any) =
    o.isInstanceOf[Category] && o.asInstanceOf[Category].args.equals(this.args)
...
object Category {
  ...
  /** Construct a [[Category]]
    ...
    *   category name.
    * @param colorThe
    *   category color. Must be picked from the CategoryColor list. At least one
  ...
  implicit val codec: JsonValueCodec[Category] =
    new JsonValueCodec[Category] {
      def decodeValue(in: JsonReader, default: Category) =
        new Category(
          implicitly[JsonValueCodec[CategoryArgs]].decodeValue(in, default.args)
        )
