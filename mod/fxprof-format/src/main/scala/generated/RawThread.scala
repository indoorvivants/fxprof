...
/** The thread type. Threads are stored in an array in profile.threads. * If a
  * profile contains threads from different OS-level processes, all threads are
  * flattened into the single threads array, and per-process information is
  * duplicated on each thread. In the UI, we recover the process separation
  * based on thread.pid. * There is also a derived `Thread` type, see
  * profile-derived.js.
  */
class RawThread private (private[fxprof] val args: RawThreadArgs) {
  ...
  /** The eTLD+1 of the isolated content process if provided by the back-end. It
    * will be undefined if:
    *   - Fission is not enabled.
    *   - It's not an isolated content process.
    *   - It's a sanitized profile.
    *   - It's a profile from an older Firefox which doesn't include this field
  ...
  /** If present and true, this thread was launched for a private browsing
    * session only. When false, it can still contain private browsing data if
    * the profile was captured in a non-fission browser. It's absent in Firefox
    * 97 and before, or in Firefox 98+ when this thread had no extra attribute
    * at all.
    */
  def isPrivateBrowsing: Option[Boolean] = args.isPrivateBrowsing

  /** If present and non-0, the number represents the container this thread was
    * loaded in. It's absent in Firefox 97 and before, or in Firefox 98+ when
    * this thread had no extra attribute at all.
    */
  ...
  override def equals(o: Any) = o
    .isInstanceOf[RawThread] && o.asInstanceOf[RawThread].args.equals(this.args)
  ...
  implicit val codec: JsonValueCodec[RawThread] =
    new JsonValueCodec[RawThread] {
      def decodeValue(in: JsonReader, default: RawThread) =
        new RawThread(
          implicitly[JsonValueCodec[RawThreadArgs]]
